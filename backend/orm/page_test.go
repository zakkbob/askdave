...
func TestCreatePage_And_PageByUrl_And_PageByID(t *testing.T) {
	...
	tmpDsts, err := url.ParseMany([]string{"https://www.testcreatepageandbyurlbyidlink1.com/path/e", "https://www.testcreatepageandbyurlbyidlink2.com"})
	assert.NoError(t, err, "ParseMany should not return an error")

	var dsts []url.URL
	for _, u := range tmpDsts {
		dsts = append(dsts, *u)
	}
	...
	t.Run("PageByID", func(t *testing.T) {
